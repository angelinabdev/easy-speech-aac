...
export default function MoodTab({ moodData, onUpdate }: MoodTabProps) {
  ...
  const printMoodData = async () => {
    ...
    if (printWindow) {
      ...
      printWindow.document.write('<style>body { font-family: sans-serif; } h1, h2 { text-align: center; } table { margin-top: 20px; border-collapse: collapse; width: 100%; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #f2f2f2; }</style>');
      ...
      if (chartData.some(d => d.count > 0)) {
        ...
        printWindow.document.write('<p>No analytics data recorded.</p>');
      }
            ...
            <CardHeader>
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <CardDescription>This data is saved to your account and available on any device.</CardDescription>
                    </div>
                     <div className="flex items-center gap-2">
                        ...
                        <AlertDialog>
                            <AlertDialogTrigger asChild>
                                <Button variant="destructive" size="sm" disabled={moodHistory.length === 0}>
